...
class SearchPagination<Item: Paginable>: ObservableObject {
    ...
    @Published var scope: SearchScope
    @Published var previousQuery: String = ""
    ...
    func startObservable() {
        ...
        self.$query
            .map { [weak self] query in
                ...
                if self.previousQuery != trimmedQuery {
                    self.items.removeAll()
                    self.state = trimmedQuery.isEmpty ? .idle : .loadingInitial
                    self.page = 1
                }
